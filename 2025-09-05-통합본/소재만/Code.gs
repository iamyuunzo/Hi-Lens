// ================================================================
//   [v6.2 FINAL] Code.gs (최종 안정화 버전)
// ================================================================

const SPREADSHEET_ID = "1AZ_JGeICcNaHQHQC6BoT_2XuhQhnTkwvzn1CxLPJFBQ";
...
function processDetailUrlWithGemini(detailUrl) {
  ...
  try {
    ...
    const content = JSON.parse(apiResponse.getContentText()).candidates[0].content.parts[0].text;
    const jsonMatch = content.match(/\{[\s\S]*\}/);
    if (!jsonMatch) {
      Logger.log(`🔴 [JSON 파싱 실패] AI 응답에서 유효한 JSON을 찾지 못했습니다. 응답: ${content}`);
      return false;
    }
